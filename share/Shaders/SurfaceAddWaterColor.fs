...
uniform vec3 waterColor; // New uniform for water color
uniform vec3 waterReflectionColor; // New uniform for water reflection color
...
void addWaterColor(in vec2 fragCoord,inout vec4 baseColor)
	...
	if(waterLevel>snowLevel+1.0e-4)
		...
		float specular=pow(dot(wn,normalize(vec3(0.075,0.075,1.0))),100.0);
		
		/* Apply water color as base with colored specular highlights */
		vec4 finalWaterColor=vec4(waterColor + waterReflectionColor * specular, 1.0);
		
		/* Mix the water color with the base surface color based on the water level: */
		baseColor=mix(baseColor,finalWaterColor,min((waterLevel-snowLevel)*waterOpacity,1.0));
		}
	}
