...
Sandbox::Sandbox(int& argc,char**& argv)
	...
	/* DEBUG: Add logging before tool initialization */
	std::cout << "\n=== DEBUG: Starting tool initialization ===" << std::endl;
	
	/* Initialize the custom tool classes: */
	std::cout << "DEBUG: Initializing GlobalWaterTool..." << std::endl;
	GlobalWaterTool::initClass(*Vrui::getToolManager());
	std::cout << "DEBUG: GlobalWaterTool initialized successfully" << std::endl;
	
	std::cout << "DEBUG: Initializing LocalWaterTool..." << std::endl;
	LocalWaterTool::initClass(*Vrui::getToolManager());
	std::cout << "DEBUG: LocalWaterTool initialized successfully" << std::endl;
	
	std::cout << "DEBUG: Initializing DEMTool..." << std::endl;
	DEMTool::initClass(*Vrui::getToolManager());
	std::cout << "DEBUG: DEMTool initialized successfully" << std::endl;
	
	if(waterTable!=0)
		{
		std::cout << "DEBUG: Initializing BathymetrySaverTool..." << std::endl;
		BathymetrySaverTool::initClass(waterTable,*Vrui::getToolManager());
		std::cout << "DEBUG: BathymetrySaverTool initialized successfully" << std::endl;
		}
	
	/* DEBUG: Check if ScriptExecutorTool can be loaded */
	std::cout << "\n=== DEBUG: Checking ScriptExecutorTool ===" << std::endl;
	
	/* Get the tool manager and print some debug info */
	Vrui::ToolManager* toolManager = Vrui::getToolManager();
	std::cout << "DEBUG: Tool manager obtained: " << (toolManager != 0 ? "YES" : "NO") << std::endl;
	
	/* Try to load ScriptExecutorTool */
	std::cout << "DEBUG: Attempting to load ScriptExecutorTool..." << std::endl;
	try
		{
		Vrui::ToolFactory* scriptFactory = toolManager->loadClass("ScriptExecutorTool");
		if(scriptFactory)
			{
			std::cout << "DEBUG: ScriptExecutorTool loaded successfully!" << std::endl;
			std::cout << "  Factory name: " << scriptFactory->getName() << std::endl;
			// Cannot print ToolInputLayout directly, just note it exists
			std::cout << "  ScriptExecutorToolFactory found" << std::endl;
			}
		else
			{
			std::cout << "DEBUG: ScriptExecutorTool factory is NULL after loadClass" << std::endl;
			}
		}
	catch(const std::exception& e)
		{
		std::cout << "DEBUG: Exception loading ScriptExecutorTool: " << e.what() << std::endl;
		}
	catch(...)
		{
		std::cout << "DEBUG: Unknown exception loading ScriptExecutorTool" << std::endl;
		}
	
	/* DEBUG: List all available tool class names from configuration */
	std::cout << "\n=== DEBUG: Configured tool classes ===" << std::endl;
	
	/* Read the global Vrui configuration to see what tool classes are configured */
	try
		{
		/* Check local SARndbox.cfg */
		std::string localConfigFile = "etc/SARndbox.cfg";
		std::cout << "DEBUG: Looking for local config at: " << localConfigFile << std::endl;
		
		/* Also check if toolClassNames is configured in the merged configuration */
		std::cout << "DEBUG: Checking if toolClassNames contains ScriptExecutorTool..." << std::endl;
		// Note: We can't easily access Vrui's internal configuration here,
		// but we've already tried to load it above
		}
	catch(const std::exception& e)
		{
		std::cout << "DEBUG: Exception checking configuration: " << e.what() << std::endl;
		}
	
	std::cout << "=== DEBUG: Tool initialization complete ===" << std::endl << std::endl;
...
void Sandbox::frame(void)
	{
	/* DEBUG: Check for key events - this is where we would catch 'n' and 'p' keys */
	static bool keyDebugPrinted = false;
	if(!keyDebugPrinted)
		{
		std::cout << "DEBUG: frame() method called - key event handling would happen here" << std::endl;
		keyDebugPrinted = true;
		}
...
void Sandbox::eventCallback(Vrui::Application::EventID eventId,Vrui::InputDevice::ButtonCallbackData* cbData)
	...
	std::cout << "DEBUG: eventCallback called - eventId=" << eventId
				       << ", button state=" << (cbData->newButtonState ? "pressed" : "released") << std::endl;
	...
	if(cbData->newButtonState)
		...
		switch(eventId)
			...
			default:
				std::cout << "DEBUG: Unhandled eventId=" << eventId << std::endl;
				break;
			}
		}
	}
