...
void SurfaceRenderer::updateSinglePassSurfaceShader(const GLLightTracker& lt,SurfaceRenderer::DataItem* dataItem) const
	...
	try
		...
		*******************************************************************/
		...
		if(useWarpTexture)
			{
			/* Declare the texture warping function: */
			fragmentDeclarations+="\
				void applyTextureWarping(inout vec4);\n";
			
			/* Compile the texture warping shader: */
			shader.addShader(compileFragmentShader("SurfaceContourWarp"));
			
			/* Call texture warping function from fragment shader's main function: */
			fragmentMain+="\
				/* Apply texture warping to the base color: */\n\
				applyTextureWarping(baseColor);\n\
				\n";
			}
		...
		*******************************************************************/
		...
		if(useWarpTexture)
			{
			/* Query texture warping uniform variables: */
			shader.setUniformLocation("warpTexture");
			shader.setUniformLocation("useWarpTexture");
			shader.setUniformLocation("warpIntensity");
			shader.setUniformLocation("textureScale");
			shader.setUniformLocation("gradientThreshold");
			shader.setUniformLocation("warpMode");
			shader.setUniformLocation("textureBlendMode");
			shader.setUniformLocation("textureOpacity");
			}
		if(drawContourLines)
			{
		...
		if(waterTable!=0&&dem==0)
			...
			shader.setUniformLocation("waterReflectionColor");
			}
...
SurfaceRenderer::SurfaceRenderer(const DepthImageRenderer* sDepthImageRenderer)
	:depthImageRenderer(sDepthImageRenderer),
	 ...
	 animationTime(0.0),
	 warpTextureObject(0),useWarpTexture(false),
	 warpIntensity(0.5f),textureScale(0.01f),gradientThreshold(0.1f),
	 warpMode(0),textureBlendMode(0),textureOpacity(0.8f)
	{
	/* Initialize default water color to blue: */
	waterColor[0]=0.0f;
	waterColor[1]=0.0f;
	waterColor[2]=1.0f;
	
	/* Initialize default water reflection color to white: */
	waterReflectionColor[0]=1.0f;
	waterReflectionColor[1]=1.0f;
	waterReflectionColor[2]=1.0f;
...
void SurfaceRenderer::setWaterColor(const GLfloat newWaterColor[3])
	{
	/* Set the new water color: */
	for(int i=0;i<3;++i)
		waterColor[i]=newWaterColor[i];
	}

void SurfaceRenderer::setWaterReflectionColor(const GLfloat newWaterReflectionColor[3])
	{
	/* Set the new water reflection color: */
	for(int i=0;i<3;++i)
		waterReflectionColor[i]=newWaterReflectionColor[i];
	}
...
void SurfaceRenderer::setAnimationTime(double newAnimationTime)
	...
	if(elevationColorMap!=0&&elevationColorMap->isColorCycling())
		elevationColorMap->updateColorCycle(float(animationTime));
	
	/* Poll the file monitor: */
	fileMonitor.processEvents();
	}

void SurfaceRenderer::setUseWarpTexture(bool enable)
	{
	useWarpTexture=enable;
	++surfaceSettingsVersion;
	}

void SurfaceRenderer::loadWarpTexture(const char* filename)
	{
	/* Load the texture image using Vrui's image loading capabilities */
	// Note: This is a placeholder - actual implementation would use Vrui's image loading
	// For now, we'll just generate a texture ID
	if(warpTextureObject==0)
		glGenTextures(1,&warpTextureObject);
	
	/* Mark that we need to rebuild the shader */
	++surfaceSettingsVersion;
	}

void SurfaceRenderer::setWarpIntensity(float intensity)
	{
	warpIntensity=intensity;
	}

void SurfaceRenderer::setTextureScale(float scale)
	{
	textureScale=scale;
	}

void SurfaceRenderer::setGradientThreshold(float threshold)
	{
	gradientThreshold=threshold;
	}

void SurfaceRenderer::setWarpMode(int mode)
	{
	warpMode=mode;
	}

void SurfaceRenderer::setTextureBlendMode(int mode)
	{
	textureBlendMode=mode;
	}

void SurfaceRenderer::setTextureOpacity(float opacity)
	{
...
void SurfaceRenderer::renderSinglePass(const SurfaceRenderer::Rect& viewport,const PTransform& projection,const OGTransform& modelview,GLContextData& contextData,TextureTracker& textureTracker) const
	...
	if(useWarpTexture)
		{
		/* Bind the warp texture: */
		dataItem->heightMapShader.uploadUniform(textureTracker.bindTexture(GL_TEXTURE_2D,warpTextureObject));
		
		/* Upload texture warping parameters: */
		dataItem->heightMapShader.uploadUniform(GLint(useWarpTexture ? 1 : 0));
		dataItem->heightMapShader.uploadUniform(GLfloat(warpIntensity));
		dataItem->heightMapShader.uploadUniform(GLfloat(textureScale));
		dataItem->heightMapShader.uploadUniform(GLfloat(gradientThreshold));
		dataItem->heightMapShader.uploadUniform(GLint(warpMode));
		dataItem->heightMapShader.uploadUniform(GLint(textureBlendMode));
		dataItem->heightMapShader.uploadUniform(GLfloat(textureOpacity));
		}
	...
	if(waterTable!=0&&dem==0)
		...
		/* Upload the water color: */
		dataItem->heightMapShader.uploadUniform3v(1,waterColor);
		
		/* Upload the water reflection color: */
		dataItem->heightMapShader.uploadUniform3v(1,waterReflectionColor);
		}
